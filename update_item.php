...
{
    $itemid = $_GET['itemId'];
    $sql = "SELECT itemId, itemName, description, price, itemCategory, subCategory, model, itemImage FROM items WHERE itemId = '".$itemid."'";
    $exec = mysqli_query($conn,$sql);
    while($thisrow = mysqli_fetch_assoc($exec)){
    $_SESSION['ItemId'] = $thisrow['itemId'];
    $_SESSION['ItemYouEntered'] = $thisrow['itemName'];
    $_SESSION['ItemDescriptionYouEntered'] = $thisrow['description'];
    $_SESSION['CatYouEntered'] = $thisrow['itemCategory'];
    $_SESSION['SubCatYouEntered'] = $thisrow['subCategory'];
    $_SESSION['BrandYouEntered'] = $thisrow['model'];
    $_SESSION['PriceYouEntered'] = $thisrow['price'];
    $_SESSION['currentItemPic'] = $_SESSION['origiPic'] = $thisrow['itemImage'];
    }
}
        
$name = $itemDescription = $itemCategory = $itemSubCategory = $itemBrand = $price = "";
if(isset($_POST['submitNewItem'])){
    ...
    $image = $_SESSION['ItemPicBackend'];}
     ...
     $image ="itemimages/default.png";   
    }

 //if u cyah manage the war, stay outta fight  
//check if item exists
    $q = mysqli_query($conn,"Select itemId from items WHERE itemId='{$_SESSION['ItemId']}'");
    $exists = mysqli_num_rows($q); 
if($exists = 0){
    $_SESSION['itemfail'] = "Item - ".$name." Could Not be Updated";
    header("location: store.php"); exit();

}
    elseif(($name == $_SESSION['ItemYouEntered']) && ($itemDescription == $_SESSION['ItemDescriptionYouEntered']) && ($itemCategory == $_SESSION['CatYouEntered']) && ($itemSubCategory == $_SESSION['SubCatYouEntered']) && ($itemBrand == $_SESSION['BrandYouEntered'])&& ($price == $_SESSION['PriceYouEntered'])&& ($image == $_SESSION['origiPic'])){
     
$_SESSION['itemfail'] = "No changes to be Updated for Item - ".$name;
 if(isset($_SESSION['ItemId'])){ unset($_SESSION['ItemId']);}
 ...
 if(isset($_SESSION['origiPic'])){ unset($_SESSION['origiPic']);}
  ...
  header("location: store.php"); exit();
   ...
   elseif(($price < 10) && (!is_numeric($price))){
...
$sql = "UPDATE items SET itemName = '{$name}' , description = '{$itemDescription}' , price = '{$price}'  , itemCategory = '{$itemCategory}' , subCategory = '{$itemSubCategory}' , model = '{$itemBrand}', itemImage = '{$image}' WHERE itemId = '{$_SESSION['ItemId']}'";
$sqlExec=mysqli_query($conn,$sql);
$_SESSION['updateitemsuccess'] = "Item - ".$name." Details Updated";
...
<nav class="navbar  navbar-fixed-top " style="background-color:white;">
    ...
    <div class="content" id="spec">
        ...
        <div class="row">
        <div class="col-sm-8"><h2  class="lead text-center" ><b>Update Item Details</b></h2></div>
        <div class="col-sm-4" >
    ...
    <strong>Success! </strong><?php echo $_SESSION['newitemsuccess'];?>
            ...
            <div class="panel-heading text-center lead">Update This Item</div>
...
<form id="AddItemsForm" method="post" name="AddItemsForm" role="form" class="form-horizontal form-inline" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
    ...
    <option disabled selected value="">Select SubCategory</option>
     <?php if(isset($_SESSION['SubCatYouEntered']) ){ ?>  
     <option  selected value="<?php echo $_SESSION['SubCatYouEntered'] ?>"><?php echo $_SESSION['SubCatYouEntered'] ?></option>    
   <?php } ?>
    ...
    <button type="submit" id="submitNewItem" name="submitNewItem" class="btn btn-success btn-lg btn-block">UPDATE THIS ITEM</button>
    <?php if(isset($_SESSION['itemDescriptionError']) || (isset($_SESSION['ItemError']))|| ( isset( $_SESSION['PriceYouEntered']) ) || (isset( $_SESSION['itemBrandError'])) ){ $_SESSION['page'] = "store.php" ?><a  href="cancel_addnewitem.php" id="CancelNewItem" name="CancelNewItem" class="btn btn-warning btn-lg btn-block">CANCEL</a><?php }  ?>
    </form>
</div>
<div class="col-sm-6">
    <div style="margin-bottom:10px;">
